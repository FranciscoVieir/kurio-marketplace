import type { Nft } from '@/features/nft/types/nft'

export const nftFixtures: Nft[] = [
  {
    id: 'emerald-ape-042',
    name: 'Emerald Ape #042',
    tokenId: '#0042',
    priceEth: '1.19',
    previousPriceEth: '1.39',
    imageUrl: '/images/emerald-ape.png',
    collection: 'Kurio Apes',
    network: 'ethereum',
    category: 'digital-art',
    availableQuantity: 8,
    version: 1,
    createdAt: '2026-09-08T14:30:00.000Z',
    trendingScore: 94,
  },

  {
    id: 'sage-nomad-009',
    name: 'Sage Nomad #009',
    tokenId: '#0009',
    priceEth: '1.69',
    previousPriceEth: '1.89',
    imageUrl: '/images/sage-nomad.png',
    collection: 'Kurio Apes',
    network: 'ethereum',
    category: 'digital-art',
    availableQuantity: 6,
    version: 1,
    createdAt: '2026-09-07T11:00:00.000Z',
    trendingScore: 88,
  },

  {
    id: 'ivory-baron-088',
    name: 'Ivory Baron #088',
    tokenId: '#0088',
    priceEth: '1.79',
    previousPriceEth: '2.29',
    imageUrl: '/images/ivory-baron.png',
    collection: 'Kurio Apes',
    network: 'polygon',
    category: 'digital-art',
    availableQuantity: 9,
    version: 1,
    createdAt: '2026-09-06T09:15:00.000Z',
    trendingScore: 71,
  },

  {
    id: 'crimson-oracle-015',
    name: 'Crimson Oracle #015',
    tokenId: '#0015',
    priceEth: '0.82',
    imageUrl: '/images/crimson-oracle.png',
    collection: 'Kurio Visions',
    network: 'solana',
    category: 'generative',
    availableQuantity: 12,
    version: 1,
    createdAt: '2026-09-09T08:20:00.000Z',
    trendingScore: 96,
  },

  {
    id: 'neon-relic-031',
    name: 'Neon Relic #031',
    tokenId: '#0031',
    priceEth: '2.45',
    previousPriceEth: '2.80',
    imageUrl: '/images/neon-relic.png',
    collection: 'Kurio Relics',
    network: 'ethereum',
    category: 'collectibles',
    availableQuantity: 4,
    version: 1,
    createdAt: '2026-09-05T18:45:00.000Z',
    trendingScore: 91,
  },

  {
    id: 'solar-drift-102',
    name: 'Solar Drift #102',
    tokenId: '#0102',
    priceEth: '0.59',
    imageUrl: '/images/solar-drift.png',
    collection: 'Kurio Motion',
    network: 'polygon',
    category: '3d-art',
    availableQuantity: 18,
    version: 1,
    createdAt: '2026-09-03T16:10:00.000Z',
    trendingScore: 62,
  },

  {
    id: 'echo-frame-077',
    name: 'Echo Frame #077',
    tokenId: '#0077',
    priceEth: '3.10',
    previousPriceEth: '3.40',
    imageUrl: '/images/echo-frame.png',
    collection: 'Kurio Frames',
    network: 'ethereum',
    category: 'photography',
    availableQuantity: 3,
    version: 1,
    createdAt: '2026-09-02T12:00:00.000Z',
    trendingScore: 84,
  },

  {
    id: 'void-signal-011',
    name: 'Void Signal #011',
    tokenId: '#0011',
    priceEth: '0.34',
    imageUrl: '/images/void-signal.png',
    collection: 'Kurio Frequencies',
    network: 'solana',
    category: 'music',
    availableQuantity: 24,
    version: 1,
    createdAt: '2026-09-01T19:30:00.000Z',
    trendingScore: 58,
  },

  {
    id: 'pixel-forge-064',
    name: 'Pixel Forge #064',
    tokenId: '#0064',
    priceEth: '1.05',
    imageUrl: '/images/pixel-forge.png',
    collection: 'Kurio Arcade',
    network: 'polygon',
    category: 'games',
    availableQuantity: 10,
    version: 1,
    createdAt: '2026-08-30T10:00:00.000Z',
    trendingScore: 79,
  },

  {
    id: 'golden-pass-020',
    name: 'Golden Pass #020',
    tokenId: '#0020',
    priceEth: '4.75',
    previousPriceEth: '5.20',
    imageUrl: '/images/golden-pass.png',
    collection: 'Kurio Access',
    network: 'ethereum',
    category: 'memberships',
    availableQuantity: 2,
    version: 1,
    createdAt: '2026-08-28T13:25:00.000Z',
    trendingScore: 90,
  },

  {
    id: 'utility-core-055',
    name: 'Utility Core #055',
    tokenId: '#0055',
    priceEth: '0.95',
    imageUrl: '/images/utility-core.png',
    collection: 'Kurio Utility',
    network: 'solana',
    category: 'utility',
    availableQuantity: 15,
    version: 1,
    createdAt: '2026-08-25T17:40:00.000Z',
    trendingScore: 66,
  },

  {
    id: 'chrome-muse-093',
    name: 'Chrome Muse #093',
    tokenId: '#0093',
    priceEth: '2.15',
    previousPriceEth: '2.49',
    imageUrl: '/images/chrome-muse.png',
    collection: 'Kurio Visions',
    network: 'polygon',
    category: 'digital-art',
    availableQuantity: 7,
    version: 1,
    createdAt: '2026-09-04T07:50:00.000Z',
    trendingScore: 86,
  },
]